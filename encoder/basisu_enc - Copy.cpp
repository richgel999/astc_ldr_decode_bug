...
#error TODO
			...
			for (uint32_t x = 0; x < width; x++)
			{
				...
				if (total_chans)
				{
					for (uint32_t c = 0; c < total_chans; c++)
					{
						...
						if ((fabs(fa) > MAX_HALF_FLOAT) || (fabs(fb) > MAX_HALF_FLOAT))
	...
	void image_metrics::calc_half(const imagef& a, const imagef& b, uint32_t first_chan, uint32_t total_chans, bool avg_comp_error)
	{
		...
		for (uint32_t y = 0; y < height; y++)
		{
			for (uint32_t x = 0; x < width; x++)
			{
				...
				for (uint32_t i = 0; i < 4; i++)
				{
					...
					if ((fabs(ca[i]) > MAX_HALF_FLOAT) || (fabs(cb[i]) > MAX_HALF_FLOAT))
						m_hf_mag_overflow = true;
				...
				int cah[4] = { float_to_half(ca[0]), float_to_half(ca[1]), float_to_half(ca[2]), float_to_half(ca[3]) };
				int cbh[4] = { float_to_half(cb[0]), float_to_half(cb[1]), float_to_half(cb[2]), float_to_half(cb[3]) };
		...
		m_psnr = m_rms ? (float)clamp<double>(log10(max_val / m_rms) * 20.0f, 0.0f, 100.0f) : 100.0f;
	}
	...
	bool read_rgbe(const char* pFilename, imagef& img, rgbe_header_info& hdr_info)
	{
		...
		for (uint32_t d = 0; d < 2; d++) // 0=Y, 1=X
		{
			...
			if (cur_line.size() < 3)
				return false;
			if (!is_pos_x && !is_neg_y && !is_pos_y)
				return false;

			uint32_t& dim = is_pos_x ? width : height;
		...
		std::vector<color_rgba> temp_scanline(width);
		...
		for (uint32_t y = 0; y < height; y++)
		{
			...
			else
			{
				...
				{
					// Check B and E. If this isn't the width in network order, something is wrong. The pixel would also be denormalized, and invalid.
			...
			for (uint32_t s = 0; s < 4; s++)
			{
				...
				while (x_ofs < width)
				{
					uint32_t num_remaining = width - x_ofs;
			...
			for (uint32_t x = 0; x < width; x++, pDst++)
				hdr_convert(temp_scanline[x], *pDst);

			assert((pDst - &img(0, y)) == width);

		} // y

		// Rare enough that optimizing this case isn't important
		if (flip_y)
		{
			imagef flipped_img(width, height);

			for (uint32_t y = 0; y < height; y++)
				for (uint32_t x = 0; x < height; x++)
					flipped_img(x, height - 1 - y) = img(x, y);

			flipped_img.swap(img);
		}
	...
	// Basic global Reinhart tone mapping, output converted to sRGB with no dithering, alpha is carried through unchanged.
	void tonemap_image(image &ldr_img, imagef &hdr_img, float exposure)
	{
